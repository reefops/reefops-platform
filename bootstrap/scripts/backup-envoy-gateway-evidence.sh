...
backup_dir="${REEFOPS_ENVOY_GATEWAY_BACKUP_DIR:?Define un directorio externo}"
age_recipient="${REEFOPS_ENVOY_GATEWAY_BACKUP_RECIPIENT:?Define el recipient age}"
timestamp="$(date -u '+%Y%m%dT%H%M%SZ')"
operation_id="$(uuidgen | tr '[:upper:]' '[:lower:]')"
backup_file="${backup_dir}/envoy-gateway-evidence-${timestamp}-${operation_id}.jsonl.age"
...
if [[ ! -s "${evidence_file}" ]]; then
  echo "No existe evidencia de Envoy Gateway para respaldar." >&2
  exit 2
fi
