...
local_port="${REEFOPS_OPENBAO_CONFIG_PORT:-18202}"
temp_dir="$(mktemp -d)"
ca_file="${temp_dir}/openbao-ca.crt"
port_forward_log="${temp_dir}/port-forward.log"
port_forward_pid=""
lock_dir="${XDG_STATE_HOME:-${HOME}/.local/state}/reefops/openbao-bootstrap/configuration.lock"
lock_acquired="false"
...
if ! mkdir "${lock_dir}" 2>/dev/null; then
  echo "Ya existe una configuración OpenBao en curso." >&2
  exit 1
fi
lock_acquired="true"
...
"${project_root}/bootstrap/scripts/configure-openbao.sh"
