...
audit_dir="${REEFOPS_SEAWEEDFS_AUDIT_DIR:-${XDG_STATE_HOME:-${HOME}/.local/state}/reefops/seaweedfs}"
...
secret_path="platform/seaweedfs/s3"
access_key=""
secret_key=""
temp_dir="$(mktemp -d)"
...
finish() {
  ...
  base_record="$(
    ...
    '{
      ...
      operation: "seaweedfs-s3-credentials-bootstrap",
      actor: $actor,
...
if ! mkdir "${evidence_lock}" 2>/dev/null; then
  echo "Ya existe un bootstrap de credenciales en curso." >&2
  exit 1
fi
lock_acquired="true"
...
if bao kv metadata get -format=json "${secret_path}" \
  ...
  result="success"
  echo "La credencial S3 ya existe; no se ha leído, mostrado ni rotado."
  exit 0
fi
...
if ! bao kv put -cas=0 "${secret_path}" "@${secret_file}" >/dev/null 2>&1; then
  if bao kv metadata get -format=json "${secret_path}" \
    ...
    result="success"
    echo "Otra operación creó la credencial; no se ha rotado ni leído."
    exit 0
  fi
  echo "La creación atómica de la credencial S3 ha fallado." >&2
  exit 1
fi
...
if ! bao kv metadata get -format=json "${secret_path}" |
  jq -e '.data.current_version == 1 and .data.versions["1"].deletion_time == ""' \
    >/dev/null; then
  echo "No se pudo verificar la primera versión de la credencial S3." >&2
  exit 1
fi

disposition="created"
result="success"
echo "Credencial S3 creada una sola vez y custodiada en OpenBao."
