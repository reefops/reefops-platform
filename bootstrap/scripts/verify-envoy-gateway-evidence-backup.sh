...
encrypted_backup="${REEFOPS_ENVOY_GATEWAY_VERIFY_FILE:?Define el backup cifrado}"
encrypted_manifest="${REEFOPS_ENVOY_GATEWAY_VERIFY_MANIFEST:?Define su manifiesto cifrado}"
age_identity="${REEFOPS_ENVOY_GATEWAY_VERIFY_IDENTITY:?Define la identidad age}"
temp_dir="$(mktemp -d)"
...
manifest_component="$(jq -er '.component' "${manifest_plain}")"
manifest_record_count="$(jq -er '.record_count' "${manifest_plain}")"
minimum_retention_days="$(
  jq -er '.minimum_retention_days' "${manifest_plain}"
)"
if [[ "${manifest_component}" != "envoy-gateway-inert-foundation" ]] ||
  [[ "${record_count}" -ne "${manifest_record_count}" ]] ||
  [[ "${minimum_retention_days}" -lt 365 ]]; then
  echo "El manifiesto no conserva componente, recuento o retención." >&2
  exit 4
fi

echo "Backup cifrado de evidencia de Envoy verificado: ${record_count} registros."
